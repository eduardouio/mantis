...
from api.vehicles.CertVehicleCreateUpdateAPI import CertVehicleCreateUpdateAPI
...
class TestCertVehicleCreateUpdateAPI:
    ...
    def test_create_certification_missing_required_fields(self, client_logged):
        ...
        incomplete_data = {
            'name': 'INSPECCION VOLUMETRICA',
            # Faltan vehicle_id, date_start, date_end
        }
    ...
    def test_create_certification_invalid_date_format(self, client_logged, test_vehicle):
        """Test crear certificación con formato de fecha inválido"""
        url = '/cert_vehicle/'
        invalid_data = {
            'vehicle_id': test_vehicle.id,
            'name': 'INSPECCION VOLUMETRICA',
            'date_start': '2024-13-45',  # Fecha inválida
            'date_end': '2025-01-01',
        }
    ...
    def test_create_certification_end_date_before_start(self, client_logged, test_vehicle):
        ...
        invalid_data = {
            ...
            'name': 'INSPECCION VOLUMETRICA',
            'date_start': date.today().strftime('%Y-%m-%d'),
            'date_end': (date.today() - timedelta(days=1)).strftime('%Y-%m-%d'),  # Fecha anterior
        }
    ...
    def test_create_certification_invalid_name(self, client_logged, test_vehicle):
        ...
        invalid_data = {
            'vehicle_id': test_vehicle.id,
            'name': 'CERTIFICACION_INEXISTENTE',  # Nombre no válido
            'date_start': date.today().strftime('%Y-%m-%d'),
            'date_end': (date.today() + timedelta(days=365)).strftime('%Y-%m-%d'),
        }
    ...
    def test_get_certifications_by_vehicle(self, client_logged, test_vehicle):
        ...
        url = f'/cert_vehicle/?vehicle_id={test_vehicle.id}'
        response = client_logged.get(url)
        ...
        # Verificar que ambas certificaciones están presentes
    ...
    def test_get_all_certifications(self, client_logged, test_vehicle):
        ...
        # Verificar que nuestra certificación está en la lista

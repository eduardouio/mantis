...
class PDFBateriesCheckList(View):
    ...
    def get(self, request, equipment_id, *args, **kwargs):
        ...
        try:
            ...
            filename = (
                f"Checklist-BateriaSanitaria-{equipment.code}-"
                f'{datetime.now().strftime("%Y%m%d")}.pdf'
            )
        except ResourceItem.DoesNotExist:
            filename = f"Checklist-BateriaSanitaria-{equipment_id}-{datetime.now().strftime('%Y%m%d')}.pdf"
